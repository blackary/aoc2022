...
ME = {"A": ROCK, "B": PAPER, "C": SCISSORS}
THEM = {"X": ROCK, "Y": PAPER, "Z": SCISSORS}
...
def get_total_score(input_lines) -> int:
  ...
  for line in input_lines:
    me_letter, them_letter = line.split(" ")
...
assert get_total_score(["A Y"]) == 1 + 0
assert get_total_score(["A Z"]) == 1 + 6

assert get_total_score(["B X"]) == 2 + 6
assert get_total_score(["B Y"]) == 2 + 3
assert get_total_score(["B Z"]) == 2 + 0

assert get_total_score(["C X"]) == 3 + 0
assert get_total_score(["C Y"]) == 3 + 6
